...
@interface MailProtocolController ()

@end

@implementation MailProtocolController
@synthesize camera;
...
- (id)initWithStyle:(UITableViewStyle)style delgate:(id<MailProtocolDelegate>)delegate_ {
...
- (IBAction)back:(id)sender {
    ...
    if (newValue != -1 && origValue != newValue) {
        if (self.delegate) [self.delegate didSetMailProtocol:newValue];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *MailProtocolTableIdentifier = @"MailProtocolTableIdentifier";
    UITableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:MailProtocolTableIdentifier];
    
    if (cell == nil) {
        
        cell = [[[UITableViewCell alloc]
                 initWithStyle:UITableViewCellStyleDefault reuseIdentifier:MailProtocolTableIdentifier]
                autorelease];
    }
