...
@property (nonatomic,assign) UIButton *lightButton;
@end
  ...
  if( self ) {
    ...
    if (isCancelEnabled) {
      ...
      if (oneDMode) {
        ...
        CGRect theRect = CGRectMake((theFrame.size.width - theSize.width) / 4, cropRect.origin.y + cropRect.size.height + 20, theSize.width, theSize.height);
      ...
      
      {
          ...
          CGSize theSize = CGSizeMake(100, 50);
          CGRect theRect = CGRectMake((theFrame.size.width - theSize.width) / 4 * 3, cropRect.origin.y + cropRect.size.height + 20, theSize.width, theSize.height);
          [lightButton setFrame:theRect];  
          
          [lightButton addTarget:self action:@selector(openLED:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)cancel:(id)sender {
    ...
    if ([device hasTorch] && [device hasFlash] && device.torchMode == AVCaptureTorchModeOn) {
